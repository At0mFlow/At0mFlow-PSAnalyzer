...
$blockedExtensions = @(
    ...
    '.p12', '.pem', '.pfx', '.py', '.so', '.sql', '.sqlite', '.sqlite3',
    '.ts', '.tsx'
)
...
try {
    ...
    if (Test-Path -LiteralPath (Join-Path $repositoryRoot '.git')) {
        ...
        if ($remotes -contains 'origin') {
            ...
            $validRemote = $origin -match '^(?:https://github\.com/At0mFlow/At0mFlow-PSAnalyzer(?:\.git)?|git@github\.com:At0mFlow/At0mFlow-PSAnalyzer\.git)$'
            if (-not $validRemote) {
